...
Этот код обрабатывает только `newState` аргумент и полностью игнорирует `status`. Однако, в многих случаях это код работает и не совсем ошибочен. Действительно, после подключения, следующее что нужно сделать, это вызвать `discoverServices()`. А в случае отключения - необходимо сделать вызов `close()`, чтобы Android освободил все связанные ресурсы в стеке Bluetooth. Эти два момента очень важные для стабильной работы BLE под Android, давайте их обсудим прямо сейчас!
...
Здесь видно что клиент `6` был зарегистрирован после вызова `connectGatt()`. Максимальный лимит клиентов (подключения) у Android равен 30 (константа `GATT_MAX_APPS` в исходниках), при достижении которого – Android не будет подключаться в устройствам вообще и вы будете получать ошибку постоянно. Достаточно странно, но сразу после загрузки Android уже имеет 5 или 6 клиентов, предполагаю, что Android их использует как-то внутри себя. Таким образом, если вы не вызываете метод `close()`, то счетчик клиентов увеличивается каждый раз при вызове `connectGatt()`. Когда вы вызываете `close()`, Bluetooth стек удаляет ваш колбек и счетчик клиентов уменьшается на единицу и освобождает ресурсы клиента. 
