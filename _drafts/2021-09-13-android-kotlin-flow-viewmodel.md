...
- передает и берет данные только только на главном потоке (main thread). Интересно, что оператор `map`

Он обрабатывает только передачу данных в основной поток и обратно, оставляя бремя управления фоновыми потоками разработчику.
Примечательно, что оператор map() выполняет свою функцию преобразования в основном потоке и не может использоваться для выполнения операций ввода-вывода или тяжелой работы процессора. В этом случае необходимо использовать оператор switchMap() в сочетании с ручным запуском асинхронной операции в фоновом потоке, даже если в основной поток нужно отправить только одно значение.
